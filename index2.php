...
<nav aria-label='breadcrumb' class=''>
  <ol class='breadcrumb bg-dark  m-4 p-4'>
    <li class='breadcrumb-item active ' ><a class='text-light h5'href='index2.php'>&nbspHome&nbsp</a></li>
<?php
...
echo"<div class='container'>
  ...
  <thead>
    <tr>
      ...
      <th ><button form='filemanag'  name='paste' class='btn text-warning ' type='button' ><input type='hidden' name='paste' value='$item'><i class='fas fa-paste'></i></button></th>
    </tr>
  </thead>
  <tbody class=''>";
...
<form id='filemanag' method='post' onsubmit='return submitResult();'>

        <td><button  name='copy' class='btn text-primary' type='button' ><input type='hidden' name='copy' value='$item'><i class='fas fa-copy'></i></button></td>

        <td><button  name='cut' class='btn text-danger' type='button' ><input type='hidden' name='cut' value='$item'><i class='fas fa-cut'></i></button></td>

        <td><button  name='delete' class='btn text-danger' type='submit'  ><input type='hidden' name='delete' value='$item'><i class='fas fa-trash-alt'></i></button></td>
        </tr></form>
        ";
     }
...
// sinon c'est un fichier on les affiches avec un lien pour afficher leur contenu si le type le permet
       ...
       $cwd="home".DIRECTORY_SEPARATOR;
        echo "
        <tr><td><i class='fas fa-file text-danger'></i> <a class='text-white ' href='".$cwd.$item."'>$baseitem</a></td>
        <td>$size</td>
        <td>$type</td>
        <td>$date</td>
